...
window.addEventListener('load', function () {
    ...
    function creatCard() {
        cardINrofm.forEach((el) => {
            ...
            imgCard.setAttribute('src', el['foto']);
            title.textContent = el['h3']
            text.textContent = el['p']

            for (let i = 0; i < 2; i++) {
                const li = document.createElement('li');
                // li.setAttribute('class', 'liText');
                li.innerHTML = el[`li${i + 1}`];
                li.style.cssText = `
            ...
            imgUser.setAttribute('src', el['userImg']);
            ...
            card.style.cssText = `
                ...
                border-radius: 15px;
                box-shadow: 0 0 20px rgb(8, 8, 12);
